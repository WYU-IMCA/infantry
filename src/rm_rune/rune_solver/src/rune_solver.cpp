...
{
  /// @brief 构造函数
  /// @param sr_params ROS2相关参数
  /// @param tf2_buffer tf2 buffer指针
  ...
  /// @brief 初始化EKF并解算出初始观测角度
  /// @param received_target 能量机关二维数据信息
  /// @return 初始观测角度
  ...
  /// @brief 更新EKF并解算出观测更新角度
  /// @param received_target 能量机关初始目标数据
  /// @return 观测更新角度
  ...
  /// @brief 预测观测角度
  /// @param predicted_position 预测的三维能量机关姿态
  /// @param timestamp 要预测的时间戳
  ...
  /// @brief 姿态解算
  /// @param predicted_target 能量机关初始目标数据pts[0]为R标
  /// @return 四维的风车姿态[三行三列的旋转矩阵 最后一列的0,1,2个数据为平移向量]
  ...
  rm_interfaces::msg::GimbalCmd RuneSolver::solveGimbalCmd(const Eigen::Vector3d &target)
  {
    ...
    trajectory_compensator->iteration_times = 20;
  ...
  double RuneSolver::getObservedAngle(double normal_angle)
  {
    ...
    if (std::abs(angle_diff) > rune_solver_params.angle_offset_thres)//angle_offset_thres 0.78
    {
  ...
  /// @brief 接口
  /// @param angle_diff 预测的角度与最后一次更新到的差角
  /// @return 待打击三维目标信息xyz
