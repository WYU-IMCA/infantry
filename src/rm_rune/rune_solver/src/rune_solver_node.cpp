...
namespace fyt::rune
{
  /// @brief 节点构造函数 初始化所有嵌入类
  /// @param options ROS2启动参数
  RuneSolverNode::RuneSolverNode(const rclcpp::NodeOptions &options) : Node("rune_solver", options)
  {
    ...
    // Timer 200 Hz
    pub_timer_ = this->create_wall_timer(std::chrono::milliseconds(5),
  ...
  /// @brief 目标解算回调函数
  /// @param rune_target_msg 能量机关二维目标数据
