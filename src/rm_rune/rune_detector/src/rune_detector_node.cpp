...
namespace imca::rune
{
      ...
      std::vector<rclcpp::Parameter> parameters)
  {
    ...
    for (const auto &param : parameters)
    {
      ...
      {
        detect_color_ = static_cast<EnemyColor>(param.as_int());
      }
    }
      ...
      std::shared_ptr<rm_interfaces::srv::SetMode::Response> response)
  {
    ...
    case VisionMode::SMALL_RUNE:
    {
      is_rune_ = true;
      is_big_rune_ = false;
      detect_color_ = static_cast<EnemyColor>(this->get_parameter("detect_color").as_int());
      RCLCPP_ERROR(this->get_logger(),"color:%ld",(this->get_parameter("detect_color").as_int()));
      createImageSub();
      break;
    }
