...
namespace fyt::serial_driver
{
      ...
      : Node("serial_driver", options)
  {
    ...
    bool use_timer = this->declare_parameter("use_timer", false);

    if (use_timer)
      // Task Timer callback
      timer_ = this->create_wall_timer(std::chrono::microseconds(4), std::bind(&SerialDriverNode::listenTimerCallback, this));
    else
      // Task thread
  ...
  void serial_driver::SerialDriverNode::listenTimerCallback()
  {
    ...
    if (protocol_->receive(receive_data))
    {
      ...
      double roll = receive_data.roll;
      double pitch = receive_data.pitch * M_PI / 180.0; // deg-> rad
      double yaw = receive_data.yaw;
      tf2::Quaternion q;
  ...
  void fyt::serial_driver::SerialDriverNode::setParam(SetParamsClient &client, const rclcpp::Parameter &param)
  {
    ...
    if (
        ...
        client.set_param_future.wait_for(std::chrono::seconds(0)) == std::future_status::ready)
    {
